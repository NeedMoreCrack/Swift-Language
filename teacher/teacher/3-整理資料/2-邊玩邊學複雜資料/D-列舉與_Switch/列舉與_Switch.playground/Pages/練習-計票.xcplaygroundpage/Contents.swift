...
## 練習：計票
 ...
 在「陣列與迴圈」Playground 中，你建立了一個函數來計算同學的投票票數。當時你只能問是非題，因為這樣才能用布林結記錄。
 
 你擔心如果使用字串會因為拼字錯誤造成投票出問題。但現在你學到列舉之後，你可以放心讓這套投票機制變得更複雜：
 */
...
/*:
 - callout(練習：): \
 在不手動計算票數的情況下，判定學生們比較喜歡巧克力工廠或海灘。*提示：檢查「陣列與迴圈」Playground，複習一下如何使用資料集合。*
 */
var beachVotes = 0
var factoryVotes = 0
...
/*:
### 延伸活動：
 在另一個投票中，為了選出學校吉祥物，你決定新增 `undecided` 選項：
 */
...
/*:
 - callout(實驗：): \
 在《陣列與迴圈》的計票練習中，有個延伸練習要求你編寫可計算任何布林投票結果的單一函數。是什麼讓你無法寫出用單一函數來計算 `tripDestinationVotes` 與 `mascotVotes`？
 */
...
/*:
[上一頁](@previous)  |  第 20 頁，共 21 頁  |  [下一頁：練習：Switch](@next)
 */